...
class ProxyApp:
    ...
    def save_config(self, **kwargs) -> None:
        ...
        try:
            ...
            self.config.set('Settings', 'max_connections', str(self.max_connections))
    ...
    def load_config(self) -> None:
        ...
        try:
            ...
            # Загрузка ограничения на соединения
            self.max_connections = self.config.getint('Settings', 'max_connections', 50)
    ...
    def create_gui(self) -> None:
        ...
        # Настройка лимита соединений
        conn_frame = ttk.Frame(settings_content)
        conn_frame.grid(row=1, column=0, columnspan=2, sticky=tk.W, padx=5, pady=2)
        
        ttk.Label(conn_frame, text="Макс. соединений:").pack(side=tk.LEFT)
        conn_spinbox = ttk.Spinbox(
            conn_frame,
            from_=10,
            to=100,
            width=5,
            command=lambda: self._update_max_connections(1)
        )
        conn_spinbox.pack(side=tk.LEFT, padx=(5, 0))
        conn_spinbox.set(self.max_connections)
        conn_spinbox.bind('<<Increment>>', lambda e: self._update_max_connections(1))
        conn_spinbox.bind('<<Decrement>>', lambda e: self._update_max_connections(-1))
    ...
    def _update_max_connections(self, delta: int) -> None:
        """Обновляет максимальное количество соединений."""
        self.max_connections = max(10, min(100, self.max_connections + delta))
        self.save_config(max_connections=self.max_connections)
    ...
    async def fragment_data(self, local_reader: asyncio.StreamReader, remote_writer: asyncio.StreamWriter) -> None:
        ...
        try:
            ...
            content_length = int.from_bytes(head[3:5], 'big')
            if content_length > 16384:  # Максимальный размер фрагмента SSL
                content_length = 16384
            
            # Читаем содержимое
            data = await local_reader.read(content_length)
            if not data:
                return
            ...
            if any(site in data for site in self.BLOCKED):
                ...
                # Фрагментируем данные на более мелкие части
                offset = 0
                while offset < len(data):
                    # Случайный размер фрагмента от 1 до 1500 байт
                    fragment_size = random.randint(1, min(1500, len(data) - offset))
                    fragment = data[offset:offset + fragment_size]
                    
                    # Формируем SSL-заголовок для фрагмента
                    fragment_header = (
                        head[0:1] +  # SSL версия
                        head[1:3] +  # SSL подверсия
                        fragment_size.to_bytes(2, 'big')  # Длина фрагмента
                    )
                    
                    # Отправляем фрагмент
                    remote_writer.write(fragment_header + fragment)
    ...
    async def handle_connection(self, local_reader: asyncio.StreamReader, local_writer: asyncio.StreamWriter) -> None:
        """Обрабатывает входящие соединения с ограничением на количество подключений."""
        ...
        if self.current_connections >= self.max_connections:
            self.log_message(f"Достигнут лимит соединений ({self.max_connections}). Отклонено: {conn_id}")
            local_writer.close()
    ...
    async def run_server(self) -> None:
        ...
        try:
            ...
            self.server = server
            self.log_message(f"Сервер запущен на localhost:8881 (макс. соединений: {self.max_connections})")
